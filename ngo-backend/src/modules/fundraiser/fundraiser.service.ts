...
export class FundraiserService {
  ...
  async getFundRaiserStatusByEmail(email: string) {
    var fundraiser = await this.fundRaiserRepository.findOne({
      where: { email: email },
    });
  ...
  async changePassword(req, changePasswordDto: ChangePasswordDto) {
    ...
    const fundraiser2 = await this.fundRaiserRepository.findOne({
      where: { email: fundraiser.email },
      select: ['password', 'fundraiser_id'],
    });
    var isSame = await bcrypt.compare(
      changePasswordDto.oldPassword,
      fundraiser2.password,
    );
    if (isSame) {
      if (changePasswordDto.newPassword == changePasswordDto.confirmPassword) {
        ...
        return this.fundRaiserRepository.update(fundraiser2.fundraiser_id, {
  ...
  async updateFundRaiserById(req, updateFundRaiserDto: UpdateFundraiserDto) {
    ...
    const updatedFund = await this.fundRaiserRepository.update(
  ...
  async getFundraiserPage(fundraiser) {
    ...
    return await this.fundraiserPageRepository.findOne({
      select: ['fundraiser'],
  ...
  async findMany(dto: FindDonationsDto, req) {
    const { payment_option, payment_status, donation_id, from_date, to_date } =
      dto;
